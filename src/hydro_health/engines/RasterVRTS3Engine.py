...
def _process_single_digitalcoast_mask(params: list) -> list[str, str, str]:
    ...
    try:
        ...
        if warped_vrt_ds is None:
            raise RuntimeError(f"Warp failed for {geotiff_prefix}")

        projection_wkt = warped_vrt_ds.GetProjection()
        spatial_ref = osr.SpatialReference(wkt=projection_wkt)
        datum_code = spatial_ref.GetAuthorityCode('DATUM') or "Unknown"
        
        # Cleanup datasets before upload
        warped_vrt_ds = None 
        src_ds = None 

        # Upload the clean VRT to S3
        geotiff_parent = '/'.join(geotiff_prefix.split('/')[1:-1])
        s3_vrt_key = f"{geotiff_parent}/mask_{geotiff_stem}.vrt"
        s3_client = boto3.client('s3')
        s3_client.upload_file(local_vrt_path, s3_bucket, s3_vrt_key)
        
        final_s3_vrt_path = f"/vsis3/{s3_bucket}/{s3_vrt_key}"

        # Determine Provider for the clean_key
        parts = geotiff_prefix.split('/')
        geotiff_folders = '/'.join(parts[1:])
        p_path = pathlib.Path(geotiff_folders)
        provider = parts[-2] if data_folder else (p_path.parents[2].name if 'dem' in geotiff_folders else p_path.parents[3].name)
...
class RasterVRTS3Engine(Engine):
    ...
    def build_output_vrts(self, output_prefix: str, file_type: str, output_geotiffs: dict, temp_output_path: pathlib.Path) -> None:
        ...
        for crs_key, tile_dict in output_geotiffs.items():
            ...
            gdal.BuildVRT(str(vrt_filename), vrt_tiles, outputSRS='EPSG:32617')
    ...
    def create_raster_vrts(self, file_type: str, ecoregion: str, data_type: str, data_folder: False, skip_existing=False) -> None:
        ...
        if data_type == 'BlueTopo':
            ...
            for provider in provider_folders:
                ...
                geotiffs = [f for f in geotiffs if not pathlib.Path(f).name.startswith('mask_')]
                output_geotiffs = self.get_digitalcoast_masks(geotiffs, data_folder)
                with tempfile.TemporaryDirectory() as temp_dir:
                    temp_output_path = pathlib.Path(temp_dir) 
                    self.build_output_vrts(output_prefix_folder, file_type, output_geotiffs, temp_output_path)
    ...
    def get_digitalcoast_masks(self, geotiffs: list, data_folder) -> dict:
        """Dask processing to build DigitalCoast Physical Binary Masks"""

        s3_bucket = get_config_item('SHARED', 'OUTPUT_BUCKET')
        task_params = [(geotiff, self.all_crs, s3_bucket, data_folder) for geotiff in geotiffs]
        
        futures = self.client.map(_process_single_digitalcoast_mask, task_params)
        results = self.client.gather(futures)
        valid_results = [r for r in results if r is not None]

        output_geotiffs = {}
        for clean_key, s3_mask_path, wkt in valid_results:
            if clean_key not in output_geotiffs:
                output_geotiffs[clean_key] = {
                    'crs': osr.SpatialReference(wkt=wkt), 
                    'tiles': []
