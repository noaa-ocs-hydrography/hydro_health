...
def get_ecoregion_tiles(param_lookup: dict[str]) -> gpd.GeoDataFrame:
    ...
    if param_lookup['drawn_polygon'].value:
        ...
        selected_sub_grids = gpd.read_file(master_grid_geopackage, layer=get_config_item('SHARED', 'ECOGRIDTILES'), columns=['Tile_ID'], mask=drawn_layer_gdf)
    else:
        ...
        selected_sub_grids = gpd.read_file(master_grid_geopackage, layer=get_config_item('SHARED', 'MODELSUBGRIDTILES'), columns=['Tilename'], mask=selected_ecoregions)

    mask_tiles = gpd.read_file(master_grid_geopackage, layer=get_config_item('SHARED', 'MODELSUBGRIDTILES'), columns=['Tilename'], mask=selected_sub_grids)
