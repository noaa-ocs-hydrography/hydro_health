...
def get_reef_5k_json(save: bool=False) -> dict:
    ...
    if save:
        for key, value in reef_data.items():
            ...
            with open(OUTPUTS / f'global_reef_{key}.geojson', 'w') as features:
...
def convert_json_to_shp():
    ...
    reef_polygons_json = str(OUTPUTS / 'global_reef_polygons.geojson')
    ...
    global_reef_polygons = str(OUTPUTS / 'global_reef_polygons_5k.shp')
...
def get_reef_1km_data() -> dict:
    ...
    zip_file = OUTPUTS / 'global_reefs_1km.zip'
    zip_folder = OUTPUTS / 'global_reefs_1km'
