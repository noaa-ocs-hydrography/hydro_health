...
#1 - extract the temporal trend of change over 10 years from all of the training data 
#2 - there will be 2 outputs from this 10 year trend, the ACTUAL AVG trend, which is directly from the trainiing and data, AND, 
# the PREDICTED AVG trend, which comes from the model training data, and what it predicts is the 10 year trend from all the input data. 
#3 -now we have the 10 year trends, we try and find the same relationships of those 10 year trends, and the conditions under which they occur, within P.data (the predictions extent)
# all model training / prediction / 10 year trend extraction has been over the extent of the training data (T.data) so far. 
#4 - so use the prediction sub grid (subdivided tiles), and find the relationships from P.data within that tile, match it to the trend data, and perform 
# the prediction and save it as a raster file. 

# Create Sub-grid over prediction extent----
# # Function to process grid tiles and prepare sub-grids
split_tile <- function(tile) {
  bbox <- st_bbox(tile)
  mid_x <- (bbox["xmin"] + bbox["xmax"]) / 2
...
prepare_subgrids <- function(grid_gpkg, prediction_data, prediction_mask, output_dir) {
  ...
  # Convert prediction data to spatial features
  training_sf <- st_as_sf(prediction_data, coords = c("X", "Y"), crs = st_crs(prediction_mask))
  
  # Generate sub-grids by splitting tiles
  sub_grids <- do.call(rbind, lapply(1:nrow(grid_tiles), function(i) split_tile(grid_tiles[i, ])))
  
  # Filter intersecting sub-grids
  intersecting_sub_grids <- st_filter(sub_grids, st_union(training_sf))
  
  # Add SpatExtent column as a separate object (not in sf)
  spat_extents <- lapply(intersecting_sub_grids$geometry, function(geom) {
    bbox <- st_bbox(geom) # Get the bounding box from the geometry
    ext(c(bbox["xmin"], bbox["xmax"], bbox["ymin"], bbox["ymax"])) # Convert to SpatExtent
  })
  
  # Save the resulting sub-grids to a GeoPackage (excluding spat_extent)
  st_write(intersecting_sub_grids, file.path(output_dir, "intersecting_pred_sub_grids.gpkg"))
  
  # Optionally save spat_extents separately, e.g., as an RDS file
  saveRDS(spat_extents, file.path(output_dir, "intersecting_pred_sub_grids_spat_extents.rds"))
  
  cat("Sub-grids prepared:", nrow(intersecting_sub_grids), "\n")
  return(intersecting_sub_grids)
}
# now creates extent object using terra
...
# Calculate Average Trends over 10 years and predict###---- 
# --- Define Parameters ---
...
prediction.mask <- raster("N:/HSD/Projects/HSD_DATA/NHSP_2_0/HH_2024/working/Pilot_model/prediction.mask.tif")
training.mask <- raster("N:/HSD/Projects/HSD_DATA/NHSP_2_0/HH_2024/working/Pilot_model/training.mask.tif") # for reference CRS of training grid
load("N:/HSD/Projects/HSD_DATA/NHSP_2_0/HH_2024/working/Pilot_model/data/pm.training.dataset.010325.Rdata") # T.data 
load("N:/HSD/Projects/HSD_DATA/NHSP_2_0/HH_2024/working/Pilot_model/data/pm.prediction.dataset.010325.Rdata") #P.data 
spat_extents <- readRDS(file.path(output_dir, "intersecting_pred_sub_grids_spat_extents.rds"))
# Extra 
...
# 1. Create Tile Folders:
#   - Reads Tile_ID from prediction_sub_grids and creates folders for each tile in output_dir.
# 2. Subset Data for Each Tile:
#   - Filters P_data to match the extent of each tile using the bounding box of prediction_sub_grids.
# 3. Calculate Survey Age:
#   - Computes survey_age as current_year - survey_end_date.
# 4. Predict Changes:
#   - Matches P_data predictors (bt.bathy, bt.slope, etc.) with trend_data using tolerances.
#   -Calculates predicted_depth_change and actual_depth_change.
# 5. Save Outputs:
#   Writes predictions to an .rds file and raster files for each tile.

trend_data <- trend_results

process_bathymetry_data <- function(P.data, trend_results, T.data) {
  # Ensure inputs are data frames
  if (!is.data.frame(P.data) | !is.data.frame(trend_results) | !is.data.frame(T.data)) {
    stop("All inputs must be data frames")
  }
  
  # Validate required columns
  required_columns <- list(
    P.data = c("X", "Y", "FID", "bt.bathy", "bt.slope", "grain_size", "sediment", "survey_end_date"),
    trend_results = c("FID", "X", "Y", "pred_avg_change", "actual_avg_change"),
    T.data = c("X", "Y", "FID", "bathy_2004", "bathy_2022", "slope_2004", "slope_2006", "slope_2010", "slope_2015", "slope_2022")
  )
  
  for (dataset_name in names(required_columns)) {
    dataset <- get(dataset_name)
    missing_cols <- setdiff(required_columns[[dataset_name]], names(dataset))
    if (length(missing_cols) > 0) {
      stop(paste(dataset_name, "is missing required columns:", paste(missing_cols, collapse = ", ")))
    }
  }
  
  # Process P.data
  P.data_summary <- P.data %>%
    group_by(FID) %>%
    summarise(
      mean_bt_bathy = mean(bt.bathy, na.rm = TRUE),
      mean_bt_slope = mean(bt.slope, na.rm = TRUE),
      grain_size = first(grain_size),
      sediment = first(sediment),
      survey_end_date = first(survey_end_date),
      .groups = "drop"
    )
  
  # Process trend_results
  trend_summary <- trend_results %>%
    group_by(Tile_ID) %>%
    summarise(
      pred_mean_change = mean(pred_avg_change, na.rm = TRUE),
      actual_mean_change = mean(actual_avg_change, na.rm = TRUE),
      .groups = "drop"
    )
  
  # Process T.data
  T.data_summary <- T.data %>%
    summarise(
      bathy_range_2004_2022 = max(bathy_2022, na.rm = TRUE) - min(bathy_2004, na.rm = TRUE),
      mean_slope_all_years = rowMeans(select(., starts_with("slope_")), na.rm = TRUE) %>%
        mean(na.rm = TRUE), # Mean of all slopes across years
      .groups = "drop"
    )
  
  # Return processed results
  list(
    P.data_summary = P.data_summary,
    trend_summary = trend_summary,
    T.data_summary = T.data_summary
  )
}


Sys.time()
result <- process_bathymetry_data(P.data, trend_results, T.data)
Sys.time()


bathy_summary <- result

# Access summaries
P.data_summary <- result$P.data_summary
trend_summary <- result$trend_summary
T.data_summary <- result$T.data_summary
...
predict_changes_by_tile <- function(P_data, trend_data, prediction_sub_grids, current_year, output_dir) {
  library(dplyr)
  library(sf)
  library(raster)
  
  message("Step 2: Predicting Changes by Tile...")
  
  # Ensure prediction_sub_grids column names align with expectations
  if (!"Tile_ID" %in% colnames(prediction_sub_grids)) {
    colnames(prediction_sub_grids)[colnames(prediction_sub_grids) == "tile"] <- "Tile_ID"
  }
  
  # Create output folders for each tile
  if (!dir.exists(output_dir)) dir.create(output_dir)
  tiles <- prediction_sub_grids$Tile_ID
  for (tile in tiles) {
    tile_folder <- file.path(output_dir, tile)
    if (!dir.exists(tile_folder)) dir.create(tile_folder)
  }
  
  # Process each tile
  for (tile in tiles) {
    message(paste("Processing Tile:", tile))
    tile_extent <- prediction_sub_grids %>% filter(Tile_ID == tile)
    
    # Subset P_data to this tile's extent
    tile_data <- P_data %>%
      filter(
        X >= st_bbox(tile_extent)$xmin,
        X <= st_bbox(tile_extent)$xmax,
        Y >= st_bbox(tile_extent)$ymin,
        Y <= st_bbox(tile_extent)$ymax
      )
    
    if (nrow(tile_data) == 0) {
      message(paste("No data found for Tile:", tile, "- Skipping."))
      next
    }
    
    message(paste("Tile", tile, "contains", nrow(tile_data), "rows."))
    
    # Calculate survey age
    tile_data <- tile_data %>%
      mutate(survey_age = pmax(0, current_year - survey_end_date))  # Ensure no negative ages
    
    # Match and predict changes
    tile_data <- tile_data %>%
      rowwise() %>%
      mutate(
        match = list(
          trend_data %>%
            filter(
              between(bt.bathy, bt.bathy - 0.5, bt.bathy + 0.5),
    ...
    output_file_rds <- file.path(output_dir, tile, paste0("predicted_changes_", tile, ".rds"))
    saveRDS(tile_data, file = output_file_rds)
    message(paste("Predictions saved for Tile:", tile, "to", output_file_rds))
    
    # Create raster for the tile
    raster_predicted <- rasterFromXYZ(tile_data %>% select(X, Y, predicted_depth_change))
    raster_actual <- rasterFromXYZ(tile_data %>% select(X, Y, actual_depth_change))
    
    output_file_pred_tif <- file.path(output_dir, tile, paste0("predicted_depth_change_", tile, ".tif"))
    output_file_actual_tif <- file.path(output_dir, tile, paste0("actual_depth_change_", tile, ".tif"))
    
    writeRaster(raster_predicted, output_file_pred_tif, format = "GTiff", overwrite = TRUE)
    writeRaster(raster_actual, output_file_actual_tif, format = "GTiff", overwrite = TRUE)
    
    message(paste("Raster files saved for Tile:", tile))
  }
  
  message("Step 2: Predicting Changes by Tile Complete.")
} # Newer but slow / ineficient

predict_changes_by_tile <- function(P_data, trend_data, spat_extents, year, output_dir, bathy_summary) {
  # Ensure input data is valid
  if (!is.list(P_data) || !is.list(trend_data) || !is.list(spat_extents)) {
    stop("P_data, trend_data, and spat_extents must be lists.")
  }
  
  # Validate that the number of spat_extents matches the expected tiles
  if (length(spat_extents) != length(P_data)) {
    stop("The number of spatial extents does not match the expected tile count.")
  }
  
  # Ensure bathy_summary contains required statistics
  if (!is.data.frame(bathy_summary) || !all(c("bathy_range_2004_2022", "mean_slope_all_years") %in% names(bathy_summary))) {
    stop("bathy_summary must be a data frame with columns 'bathy_range_2004_2022' and 'mean_slope_all_years'.")
  }
  
  # Initialize progress bar
  pbapply::pboptions(type = "timer", style = 3)
  
  # Function for processing a single tile
  process_tile <- function(tile_index) {
    tryCatch({
      # Extract the extent of the current tile
      tile_extent <- spat_extents[[tile_index]]
      
      # Ensure tile_extent is valid and is a SpatExtent object
      if (!inherits(tile_extent, "SpatExtent")) {
        stop("Tile extent is not a valid SpatExtent object.")
      }
      
      # Crop P_data and trend_data based on the tile_extent
      tile_P_data <- lapply(P_data, function(x) {
        x_extent <- terra::ext(x)  # Extract extent from each raster
        if (!is.null(terra::intersect(x_extent, tile_extent))) {
          return(terra::crop(x, tile_extent))
        } else {
          return(NULL)
        }
      }) |> compact()  # Filter out NULLs
      
      tile_trend_data <- lapply(trend_data, function(x) {
        x_extent <- terra::ext(x)  # Extract extent from each raster
        if (!is.null(terra::intersect(x_extent, tile_extent))) {
          return(terra::crop(x, tile_extent))
        } else {
      ...
      if (length(tile_P_data) == 0 || length(tile_trend_data) == 0) {
        stop("No data available for the current tile extent.")
      }
      
      # Retrieve bathy summary stats
      bathy_range <- bathy_summary$bathy_range_2004_2022
      mean_slope <- bathy_summary$mean_slope_all_years
      
      # Perform the prediction (placeholder logic, replace 'FUN' with actual function)
      predictions <- FUN(tile_P_data, tile_trend_data, year, bathy_range, mean_slope)
      
      # Save the predictions
      output_file <- file.path(output_dir, paste0("tile_", tile_index, "_predictions.tif"))
      terra::writeRaster(predictions, output_file, overwrite = TRUE)
      
      return(output_file)
    }, error = function(e) {
      message(sprintf("Error processing tile %d: %s", tile_index, e$message))
      return(NULL)
    })
  }
  
  # Use pbapply's progress bar with lapply
  results <- pbapply::pblapply(1:length(spat_extents), function(i) {
    process_tile(i)
...
predict_changes_by_tile <- function(P_data, trend_data, spat_extents, current_year, output_dir) {
  library(dplyr)
  library(terra)
  library(future.apply)
  ...
  # Function for matching and predicting changes
  predict_changes <- function(tile_data, trend_data, survey_age) {
    tile_data <- tile_data %>%
      mutate(
        pred_avg_change = map2_dbl(bt.bathy, bt.slope, ~ {
          filtered_trend <- trend_data %>%
            filter(
              between(.x, bt.bathy - 0.5, bt.bathy + 0.5),
              between(.y, bt.slope - 0.1, bt.slope + 0.1),
              grain_size == grain_size | is.na(grain_size),
              sediment == sediment | is.na(sediment)
            )
          mean(filtered_trend$pred_avg_change, na.rm = TRUE)
        }),
        actual_avg_change = map2_dbl(bt.bathy, bt.slope, ~ {
          filtered_trend <- trend_data %>%
            filter(
              between(.x, bt.bathy - 0.5, bt.bathy + 0.5),
              between(.y, bt.slope - 0.1, bt.slope + 0.1),
          ...
          mean(filtered_trend$actual_avg_change, na.rm = TRUE)
        }),
        predicted_depth_change = (pred_avg_change / 10) * survey_age,
        actual_depth_change = (actual_avg_change / 10) * survey_age
      ) %>%
      replace_na(list(predicted_depth_change = 0, actual_depth_change = 0))
    return(tile_data)
  }
  ...
  results <- future.apply::future_lapply(seq_along(spat_extents), function(tile_index) {
    tryCatch({
      tile_extent <- spat_extents[[tile_index]]
      
      # Crop P_data and trend_data based on tile_extent
      tile_P_data <- P_data %>%
        filter(X >= xmin(tile_extent), X <= xmax(tile_extent), Y >= ymin(tile_extent), Y <= ymax(tile_extent))
      
      if (nrow(tile_P_data) == 0) stop("No P_data available for the current tile extent.")
      
      tile_trend_data <- trend_data
      
      # Calculate survey age
      tile_P_data <- tile_P_data %>%
        mutate(survey_age = pmax(0, current_year - survey_end_date))
      
      # Predict changes
      predictions <- predict_changes(tile_P_data, tile_trend_data, survey_age)
      
      # Save predictions
      output_file_rds <- file.path(output_dir, paste0("tile_", tile_index, "_predicted_changes.rds"))
      saveRDS(predictions, file = output_file_rds)
      
      raster_predicted <- rasterFromXYZ(predictions %>% select(X, Y, predicted_depth_change))
      raster_actual <- rasterFromXYZ(predictions %>% select(X, Y, actual_depth_change))
      
      output_file_pred_tif <- file.path(output_dir, paste0("tile_", tile_index, "_predicted_depth_change.tif"))
      output_file_actual_tif <- file.path(output_dir, paste0("tile_", tile_index, "_actual_depth_change.tif"))
      
      terra::writeRaster(raster_predicted, output_file_pred_tif, overwrite = TRUE)
      terra::writeRaster(raster_actual, output_file_actual_tif, overwrite = TRUE)
      
      list(predictions = output_file_rds, rasters = c(output_file_pred_tif, output_file_actual_tif))
    }, error = function(e) {
      message(sprintf("Error processing tile %d: %s", tile_index, e$message))
      return(NULL)
    })
  ...
  message("Step 2: Predicting Changes by Tile Complete.")
  return(results)
}# newest - more efficient data handling and parallel processing

# Set global options
options(future.globals.maxSize = 8 * 1024^3)  # Allow up to 8 GiB for globals

# Plan for parallel processing
plan(multisession)


### Workflow Function (Runs 2 prediction functions sequentially) ###
...
# Call each function separately for debugging before running workflow
trend_results <- compute_10_year_trends(input_dir, year_pairs, output_dir, T.data)
current_year <- 2025

results <- predict_changes_by_tile(
  P_data = P.data,
  trend_data = trend_data,
  spat_extents = spat_extents,
...
output_files <- predict_changes_by_tile(
  P_data = P.data,
  trend_data = trend_results,
  prediction_sub_grids = prediction_sub_grids,
  year = 2025,
  spat_extents = spat_extents,
  output_dir = "N:/HSD/Projects/HSD_DATA/NHSP_2_0/HH_2024/working/Grid_Development/Prediction.GRID_ID"
)
