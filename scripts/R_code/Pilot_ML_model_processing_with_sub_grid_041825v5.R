...
grid_out_raster_data( # PREDICTION 
...
specific_tile_ids <- c("BH4S2574_3") #, "BH4RZ577_3") # c("BH4RZ577_2", "BH4RZ577_3", "BH4RZ577_4", "BH4RZ578_1")
...
# Gemini prediction code V2 ----

# post model re-run changes
# - new CRS
# - new boostrap data (large and predictedn) 
# -changed rugosity naming 
# ==============================================================================
...
align_predictors <- function(data_to_align, year_pair, predictors_from_boruta) {
  ...
  if (length(rugo_req) > 0) {
    ...
    cat("  - DIAGNOSTIC (align_predictors): Required rugosity cols:", paste(rugo_req, collapse=", "), "\n")
    cat("  - DIAGNOSTIC (align_predictors): All rugosity cols in data:", paste(all_rugo_cols_in_data, collapse=", "), "\n")
    cat("  - DIAGNOSTIC (align_predictors): Deduced source rugosity col:", if(length(source_rugo_col) > 0) source_rugo_col[1] else "NONE FOUND", "\n")
    
    if (length(source_rugo_col) > 0) {
      source_col_name <- source_rugo_col[1]
      message("  - INFO: Mapping from static rugosity column '", source_col_name, "'.")
      for (col in rugo_req) {
      ...
      warning(paste("Could not find a static source rugosity column to map from for tile '", data_to_align$tile_id[1], "'."))
    }
  }
  
  start_year <- as.numeric(strsplit(as.character(year_pair), "_")[[1]][1])
  
  bathy_start_name <- paste0("bathy_", start_year)
  if (bathy_start_name %in% names(data_to_align)) {
    data_to_align[, starting_bathy := .SD[[bathy_start_name]]]
  } else if ("bt.bathy" %in% names(data_to_align)) {
    data_to_align[, starting_bathy := bt.bathy]
  }
  
  slope_start_name <- paste0("slope_", start_year)
  if (slope_start_name %in% names(data_to_align)) {
    data_to_align[, starting_slope := .SD[[slope_start_name]]]
  } else if ("bt.slope" %in% names(data_to_align)) {
    data_to_align[, starting_slope := bt.slope]
  }
  ...
  if (rugo_start_name %in% names(data_to_align)) {
    ...
    source_col_name <- source_rugo_col[1]
    data_to_align[, starting_rugosity := get(source_col_name)]
  }
...
# This method now uses a robust merge and re-sorts the data to ensure
# correct spatial ordering for raster conversion.
# ------------------------------------------------------------------------------
apply_bootstrap_adjustment <- function(prediction_data, model_tile_id, year_pair, training_dir) {
  ...
  # --- FIX: Use a robust left merge to preserve the order of prediction_data ---
  # This prevents the join from scrambling the spatial order of the main dataset.
  ...
  # --- FIX 2: Re-sort the data table by spatial coordinates as a final guarantee ---
  setorderv(merged_data, c("Y", "X"), c(-1, 1))
...
match_pdp_conditions <- function(prediction_data, pdp_data, year_pair) {
  ...
  cat("  - DIAGNOSTIC: All predictors found in PDP data:", paste(all_pdp_predictors, collapse=", "), "\n")
  
  bathy_pred_name <- paste0("bathy_", start_year)
  slope_pred_name <- paste0("slope_", start_year)
  rugo_pred_name <- paste0("rugosity_", start_year)
  
  predictors_found <- list(
    bathy = if(bathy_pred_name %in% all_pdp_predictors) bathy_pred_name else NA_character_,
    slope = if(slope_pred_name %in% all_pdp_predictors) slope_pred_name else NA_character_,
    rugosity = if(rugo_pred_name %in% all_pdp_predictors) rugo_pred_name else NA_character_
  )
  
  get_range <- function(var) { if (is.na(var)) return(NA_real_); val <- env_ranges[Predictor == var, range_width]; if (length(val) == 0) return(NA_real_) else return(val) }
  bathy_rng <- get_range(predictors_found$bathy)
  slope_rng <- get_range(predictors_found$slope)
  rugosity_rng <- get_range(predictors_found$rugosity)
  
  ranges_check <- c(bathy = bathy_rng, slope = slope_rng, rugosity = rugosity_rng)
  ranges_check <- ranges_check[!is.na(unlist(predictors_found))]
  if (any(is.na(ranges_check))) { 
    missing_vars <- names(which(is.na(ranges_check)))
    stop(paste("Missing range width(s) for predictor(s):", paste(missing_vars, collapse = ", "))) 
  }
  ...
  prediction_data[, pdp_adjusted_change := {
    ...
    if (!is.na(predictors_found$bathy) && "starting_bathy" %in% names(current_point) && !is.na(current_point$starting_bathy)) {
      is_match <- is_match | (pdp_data$Predictor == predictors_found$bathy & between(pdp_data$Env_Value, current_point$starting_bathy - 0.3 * bathy_rng, current_point$starting_bathy + 0.3 * bathy_rng))
    }
    if (!is.na(predictors_found$slope) && "starting_slope" %in% names(current_point) && !is.na(current_point$starting_slope)) {
      is_match <- is_match | (pdp_data$Predictor == predictors_found$slope & between(pdp_data$Env_Value, current_point$starting_slope - 0.1 * slope_rng, current_point$starting_slope + 0.1 * slope_rng))
    }
    if (!is.na(predictors_found$rugosity) && "starting_rugosity" %in% names(current_point) && !is.na(current_point$starting_rugosity)) {
      is_match <- is_match | (pdp_data$Predictor == predictors_found$rugosity & between(pdp_data$Env_Value, current_point$starting_rugosity - 0.1 * rugosity_rng, current_point$starting_rugosity + 0.1 * rugosity_rng))
    }
...
apply_trend_adjustments_hybrid <- function(prediction_data, training_data, year_pair, k = 5) {
  ...
  potential_env_vars <- c("starting_bathy", "starting_slope", "starting_rugosity", "tsm_layer", "hurr_count", "hurr_strength")
...
compare_prediction_methods <- function(prediction_data, tile_id, year_pair, processing_mode) {
  ...
  prediction_data[, hybrid_change := fcase(
    !is.na(MeanBoots_predicted_change), MeanBoots_predicted_change,
    !is.na(pdp_adjusted_change), pdp_adjusted_change,
    !is.na(KNN_predicted_change), KNN_predicted_change,
    !is.na(XGB_predicted_change), XGB_predicted_change,
    default = NA_real_
  )]
...
# --- 11. Save Summary Plot (REVISED for Diagnostics & b.change) ---
# Creates an 8-panel summary plot to show all components and actual change data.
# ------------------------------------------------------------------------------
save_summary_plot <- function(processed_data, tile_id, year_pair, prediction_dir, crs_obj) {
  ...
  # --- FIX: Guarantee spatial order right before plotting ---
  ...
  # --- NEW: Add the actual change data to the plot list ---
  plot_list[['actual_change']] <- dt_to_raster(processed_data, "b.change_actual", crs_obj)
  
  # --- Create plot objects from the raster list ---
  p1 <- if(!is.null(plot_list$pdp)) levelplot(plot_list$pdp, main="PDP Prediction", margin=FALSE) else plot.new()
  p2 <- if(!is.null(plot_list$knn)) levelplot(plot_list$knn, main="KNN Prediction", margin=FALSE) else plot.new()
  p3 <- if(!is.null(plot_list$boots)) levelplot(plot_list$boots, main="Mean Boots Prediction", margin=FALSE) else plot.new()
  p4 <- if(!is.null(plot_list$xgb)) levelplot(plot_list$xgb, main="XGB Prediction", margin=FALSE) else plot.new()
  p5 <- if(!is.null(plot_list$bt_bathy)) levelplot(plot_list$bt_bathy, main="BT. Bathy", margin=FALSE) else plot.new()
  p6 <- if(!is.null(plot_list$survey_date)) levelplot(plot_list$survey_date, main="Survey End Date", margin=FALSE, col.regions=terrain.colors) else plot.new()
  p7 <- if(!is.null(plot_list$actual_change)) levelplot(plot_list$actual_change, main="Actual Change (b.change)", margin=FALSE) else plot.new()
  
  # Create an empty plot object to act as a placeholder
  ...
  # --- NEW: Arrange in a 4x2 grid ---
  pdf(plot_out_file, width = 20, height = 10)
  grid.arrange(grobs = list(p1, p2, p3, p4, p5, p6, p7, p_empty), ncol = 4, nrow = 2)
  dev.off()
  
  message("  - INFO: Summary plot saved to: ", plot_out_file)
}

# --- 12. Save Density Plot (REVISED for Fair Comparison) ---
# Creates a 4-panel density plot with a shared, consistent x-axis to prevent
# data from being cut off and allow for accurate visual comparison.
# ------------------------------------------------------------------------------
save_density_plot <- function(processed_data, tile_id, year_pair, prediction_dir) {
  ...
  cols_to_plot <- c("XGB_predicted_change", "pdp_adjusted_change", "KNN_predicted_change", "MeanBoots_predicted_change")
  cols_to_plot <- intersect(cols_to_plot, names(processed_data))
  
  if (length(cols_to_plot) < 2) {
    message("  - WARN: Not enough prediction types to generate a density plot.")
    return(NULL)
  }
  ...
  plot_data[, variable := factor(variable,
                                 levels = c("XGB_predicted_change", "pdp_adjusted_change", "KNN_predicted_change", "MeanBoots_predicted_change"),
                                 labels = c("XGB_change", "PDP_change", "KNN_change", "MeanBoots_change"))]
  
  # --- FIX: Calculate a global x-axis range for all plots ---
  x_range <- range(plot_data$value, na.rm = TRUE, finite = TRUE)
  
  # Function to create a single density plot with the shared x-axis
  create_density_panel <- function(data, method_name, fill_color) {
    p <- ggplot(data, aes(x=value)) + 
      geom_density(fill=fill_color, alpha=0.7) +
      coord_cartesian(xlim = x_range) + # Apply the shared x-axis range
      labs(title=paste(method_name, "Distribution")) +
      theme_minimal()
    return(p)
  }
  
  p_pdp <- create_density_panel(plot_data[variable == "PDP_change"], "PDP", "blue")
  p_knn <- create_density_panel(plot_data[variable == "KNN_change"], "KNN", "purple")
  p_xgb <- if ("XGB_change" %in% plot_data$variable) create_density_panel(plot_data[variable == "XGB_change"], "XGB", "red") else NULL
  p_boots <- if ("MeanBoots_change" %in% plot_data$variable) create_density_panel(plot_data[variable == "MeanBoots_change"], "Bootstrap Mean", "darkgreen") else NULL
  
  plot_out_dir <- file.path(prediction_dir, tile_id, "diagnostics")
  if (!dir.exists(plot_out_dir)) dir.create(plot_out_dir, recursive = TRUE)
  plot_out_file <- file.path(plot_out_dir, paste0(tile_id, "_density_plot_", year_pair, ".pdf"))
  
  pdf(plot_out_file, width = 12, height = 8)
  grid.arrange(grobs = Filter(Negate(is.null), list(p_pdp, p_xgb, p_knn, p_boots)), ncol = 2, nrow = 2)
  dev.off()
...
run_prediction_test <- function(prediction_tiles, year_pairs, crs_obj, training_dir, prediction_dir, output_dir) {
  ...
  results_list <- foreach(
    i = 1:nrow(task_grid),
    .packages = c("data.table", "dplyr", "fst", "sf", "xgboost", "FNN", "raster", "rasterVis", "gridExtra", "ggplot2", "Metrics"),
    .export = c("process_tile", "predict_elevation_change", "align_predictors",
                ...
                "save_component_rasters", "save_summary_plot", "save_density_plot",
                "run_cross_validation", "mae", "rmse")
  ) %dopar% {
  ...
  all_comparison_logs <- rbindlist(lapply(results_list, function(res) if(!is.null(res) && res$success) res$comparison_log else NULL), fill = TRUE)
  all_performance_logs <- rbindlist(lapply(results_list, function(res) if(!is.null(res) && res$success) res$performance_log else NULL), fill = TRUE)
  
  if(nrow(all_comparison_logs) > 0) fwrite(all_comparison_logs, file.path(output_dir, "prediction_method_comparison_log.csv"))
  if (nrow(all_performance_logs) > 0) fwrite(all_performance_logs, file.path(output_dir, "model_performance_log.csv"))
  ...
  for (result in results_list) {
    if (!is.null(result) && result$success) {
      ...
      if (!is.null(crs_obj)) {
        ...
        # Run cross-validation if it was a direct model run
      ...
      cat("\n--- SKIPPING SAVE for failed Tile:", result$tile_id, "| Year Pair:", result$year_pair, "Error:", result$error, "---\n")
    }
  }
...
prediction_tile_ids <- c("BH4S2574_1") #, "BH4S2572_4") # "BH4S2572_1")
