...
geoms_1 = ['_point']
repe_PaRep_query = "(quapos = '4' OR quapos = '7' OR quapos = '8') AND (status IS NULL OR status <> '18   ')"
repe_EdPd_query = "( quapos = '5  ' AND status IS NULL) OR (quapos = '5  ' AND status = '18   ') OR (quapos IS NULL AND status = '18   ')"


def extract_raw(params):
    """ Extract raw reported error data given a globals.Parameters instance """
    return Functions.extract_enc_data_p(params, features_1, geoms_1)


def raw_to_vectorprocessed(reperr):
    repe_raw = reperr.raw_filename("point")
...
def process_grid(reperr, igrid):
    repe_PaRep_vp = reperr.vector_processed_filename("PARep")
    repe_EdPd_vp = reperr.vector_processed_filename("EDPD")
    with timer("Total Time for Grid %02d = " % (igrid), "processing grid %02d" % igrid) as t:
        grid_name = "Grid_Model_Extents_%02d_Raster" % igrid
        grid_ws = reperr.ini["Input"]["grid_ws"]
        ...
        papd_rep_ed_sum_c = reperr.raster_classified_filename("pred_%02d" % igrid)

        re_out = reperr.raster_final_filename("%02dfinal" % igrid)
        ...
        t.msg("Finished PA/Rep Density and Distance Rasters")
        ...
        return parep_den_r, edpd_den_r, parep_dis_c, edpd_dis_c, papd_rep_ed_sum_c, re_out


def process_grids(reperr, grids=list(range(1, 14))):
    # RASTER
    ...
    for igrid in grids:  # Improve: Find all matching grid extents in the geodatabase
        parep_den_r, edpd_den_r, parep_dis_c, edpd_dis_c, papd_rep_ed_sum_c, re_out = process_grid(reperr, igrid)
    ...
    return parep_den_r_fns, edpd_den_r_fns, parep_dis_c_fns, edpd_dis_c_fns, papd_rep_ed_sum_c_fns, re_out_fns


def execute(ini, grids=list(range(1, 14)), use_existing_raw=True):
    reperr = globals.Parameters("RepErr", ini)
    if not use_existing_raw or not arcpy.Exists(reperr.raw_filename(geoms_1[0])):
        # RAW DATA
        with timer("Finished extracting data from ENC", "Start Extracting from ENC") as _t:
            fns = extract_raw(reperr)  # pulls the raw data from the ENCs
            if not fns:
                print("failed to make vector processed data for reported error, did ENC file not exist?")
                return
    raw_to_vectorprocessed(reperr)

    parep_den_r_fns, edpd_den_r_fns, parep_dis_c_fns, edpd_dis_c_fns, papd_rep_ed_sum_c_fns, re_out_fns = process_grids(reperr, grids)
    ...
    for filelist, outpath in [[parep_den_r_fns, parep_den_r_fn],
        ...
        with timer("Finished Mosaic to New Raster for %s" % (outname), "start mosaic %s" % (outname)) as _t:
        ...
        for fn in filelist:  # enumerate retrieves index number
            print((str(fn)))
            arcpy.Delete_management(str(fn))

    # Reclassify Reported Errors Raster
    r = arcpy.sa.Reclassify(re_final_t, "VALUE", "0 NODATA;0 1 1;1 2 2;2 3 3;3 4 4;4 5 5", "DATA")
    arcpy.Delete_management(re_final)  # save fails if data on disk in the way
    r.save(re_final)
