...
features_1 = ['.obstrn', '.pilpnt', '.uwtroc', '.wrecks']
geoms_1 = ['_point', '_line', '_polygon']
...
def raw_to_vector(haz):
    ...
    with timer("Finished Copies at") as t:
        for geom in geoms_1:
            try:
                arcpy.Select_analysis(haz.raw_filename(geom), haz.vector_filename(geom), query)
                t.msg("Finished Select " + haz.raw_filename(geom))
            except arcgisscripting.ExecuteError:
                t.msg(haz.raw_filename(geom) + " -- Unable to query input file, verify New Map is opened in ArcPro. e.g. Insert --> New Map. ")

    # Define Parameters

    vec_pt_t = haz.vector_filename("point")
    vec_line_t = haz.vector_filename("line")
    vec_pg_t = haz.vector_filename("polygon")

    # aux = Parameters("AUX", haz.ini)
    catzoc_b_r = haz.ini["AUX"]["catzoc_bcdu_raster"]

    # Output Variables
    catzoc_b_p = haz.working_filename("catzoc_bcdu_p")
    catzoc_bcdu_bounds = haz.working_filename("catzoc_bcdu_bounds")
    ...
    # @TODO -- is this raster doing anything?  Where does bcdu_f get made, SurveyQuality??
    arcpy.RasterToPolygon_conversion(catzoc_b_r, catzoc_b_p, "NO_SIMPLIFY", "VALUE")
    bcdu = Functions.generate_polygon_bounds(catzoc_b_p, catzoc_bcdu_bounds)
    # arcpy.RepairGeometry_management(catzoc_b_p)
    arcpy.Clip_analysis(vec_pt_t, catzoc_bcdu_bounds, vec_pt)
    arcpy.Clip_analysis(vec_line_t, catzoc_bcdu_bounds, vec_line)
    arcpy.Clip_analysis(vec_pg_t, catzoc_bcdu_bounds, vec_pg)


def process_grid(haz, igrid):
    with timer("Finished Classified Summed Feature Export", "Starting Grid %02d" % igrid) as t:
        vec_pt = haz.working_filename("point_VP")
        vec_line = haz.working_filename("line_VP")
        vec_pg = haz.working_filename("polygon_VP")
        grid_ws = haz.ini["Input"]["grid_ws"]
        ...
        dens_out = haz.raster_filename("%02d_den" % igrid)
        dens_c_out = haz.raster_classified_filename("%02d_dC" % igrid)
        ...
        # @todo
        # Add catzoc-driven buffer
        with timer("Finished Polygon Density Raster Classify Export") as t2:

            # Clip input polygons to grid extents
            t2.msg("Start clipping Polygons")
            arcpy.Clip_analysis(vec_pg, model_bounds, vec_pg_clip)
            t2.msg("Done clipping Polygons")
        ...
        try:
            ...
            t.msg("Finished Summed Feature Export")
        except RuntimeError:
            t.msg("Export Sum Density Numpy Array Process Failed. Filepath or Filename may be too long. Filepath is limited to 128 characters and filename is limited to 13 characters. Revise file naming structure and re-run.")
        ...
        try:
            ...
            t.msg("Export Reclassified Sum Density Numpy Array Process Failed. Filepath or Filename may be too long. Filepath is limited to 128 characters and filename is limited to 13 characters. Revise file naming structure and re-run.")

    return vec_line_clip, vec_pg_clip, vec_pg_en, dens_out, dens_c_out


def process_grids(haz, grids=list(range(1, 14))):
    den_fns = []
    den_c_fns = []
    ln_clip_fns = []
    pg_clip_fns = []
    pg_clip_en_fns = []
    for igrid in grids:  # Improve: Find all matching grid extents in the geodatabase
        vec_line_clip, vec_pg_clip, vec_pg_en, dens_out, dens_c_out = process_grid(haz, igrid)
    ...
    return ln_clip_fns, pg_clip_fns, pg_clip_en_fns, den_fns, den_c_fns


def vector_to_raster(haz, den_fns, den_c_fns):
    den_final = "navhaz_r"
    ...
    with timer("Finished Mosaic to New Raster") as t:
    ...
    arcpy.Delete_management(den_c_final_temp)  # save fails if data on disk in the way
    r_like.save(den_c_final_temp)

    # Clip final raster to EEZ bounds
    # Clip Final CATZOC_RC to EEZ Bounds
    eez = haz.ini["AUX"]["eez_buffered_ras"]
    e = arcpy.sa.ExtractByMask(den_c_final_temp, eez)
    arcpy.Delete_management(den_c_final)  # save fails if data on disk in the way
    e.save(den_c_final)


def execute(ini, grids=list(range(1, 14)), use_existing_raw=True):
    haz = globals.Parameters("KnownHazards", ini)
    if not use_existing_raw or not arcpy.Exists(haz.raw_filename(geoms_1[0])):
        # RAW DATA
        with timer("Finished extracting data from CSV", "Start Extracting from CSV") as _t:
            fns = extract_raw(haz)
            if not fns:
                print("failed to make vector processed data for groundings, did csv file not exist?")
                return
    raw_to_vector(haz)
    ln_clip_fns, pg_clip_fns, pg_clip_en_fns, den_fns, den_c_fns = process_grids(haz, grids)
    vector_to_raster(haz, den_fns, den_c_fns)

    with timer("Finished Deleting") as _t:
        for fns in ln_clip_fns, pg_clip_fns, pg_clip_en_fns, den_fns, den_c_fns:
            for fn in fns:
                if arcpy.Exists(fn):
