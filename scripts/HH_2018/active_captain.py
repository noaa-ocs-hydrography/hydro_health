...
def export_raster(grid_path, ac_vp, dens_out, dens_c_out):
...
def export_mutliple_rasters(grid_ws, ac, ac_vp, raster_numbers=list(range(1, 14))):
    # Export Classified Density Rasters for all Grids
    den_fns = []  # delete
    den_c_fns = []  # delete
    for igrid in raster_numbers:  # Improve: Find all matching grid extents in the geodatabase
        ...
        with timer("Finished Grid %02d " % (igrid), "Starting Grid %02d" % igrid) as t:
            export_raster(grid_path, ac_vp, dens_out, dens_c_out)
            den_fns.append(dens_out)
            den_c_fns.append(dens_c_out)
    return den_fns, den_c_fns


def get_ac_vector_processed(ac):
...
def execute(ini, grids=list(range(1, 14))):
    # Define Parameters
    ac = Parameters("ActiveCaptain", ini)
    ...
    grid_ws = ini["Input"]["grid_ws"]  # "r'H:\GRID\Grid.gdb'  # USER DEFINED
    ...
    # Project Data
    ac_vp = get_ac_vector_processed(ac)

    den_fns, den_c_fns = export_mutliple_rasters(grid_ws, ac, ac_vp, grids)
    ...
    arcpy.Delete_management(ac_raster_final_t)  # save fails if data on disk in the way
    r.save(ac_raster_final_t)

    # Clip final raster to EEZ Bounds
    e = arcpy.sa.ExtractByMask(ac_raster_final_t, eez_r)
    arcpy.Delete_management(ac_raster_final)  # save fails if data on disk in the way
    ...
    arcpy.Delete_management(ais_ac_rf)  # save fails if data on disk in the way
    e.save(ais_ac_rf)
    '''

    # Delete Unnecessary Files
    with timer("Finished deleting", "Deleting Intermediate + temp Rasters") as t:
        for filenames in (den_fns, den_c_fns):
            for fn in filenames:
